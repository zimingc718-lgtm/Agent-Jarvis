...
先把用户原话存进 `project/00_input/需求输入.md`，再往下派生。改动落到受控说明书之前，先在 `project/06_changes/` 立 CR。**哪怕当场就能改也要先落号**——2026-09-15 有一条反馈因为没落号，缺陷留到用户自己再撞一次才被处理。
...
**用脚本插入章节时，锚点必须锚在行首**：`text.replace("## 批准状态", ...)` 或 `partition` 认的是**第一次出现**，而这些标题名在正文里也会作为普通词出现——「`## 批准状态` 段关于……」「本文件有 `## 变化点登记` 表」都是真实存在的句子。锚错的后果是整节被插进表格中间或把一行劈成两半。写法应为 `re.search(r"(?m)^## 标题$", text)`。2026-09-14 一天之内踩了四次，最后一次踩在讲这个坑的那份记录里；`check-tables`（DEC-220 ①）接住了其中两次。
