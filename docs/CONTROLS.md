...
**声明该路线断言了用户可见/可操作的行为时，写 `机器（UI）：TEST-xxx`**（CR-20260915-process-hardening-flow）而不是普通的 `机器：TEST-xxx`。`check-ui-route` 会打开点名的测试文件核对确有 `fireEvent`/`userEvent`/`page.click` 一类控件驱动调用，没有就 FAIL——这条标记是作者自愿声明，不是从「一句话」栏猜出来的：对「一句话」做关键词匹配已经证伪过一次（45 命中里 18 条是误伤，40% 假阳性），做成硬门禁只会被当噪声无视。判定只问「兑现了吗」，不判「该不该标」；`说 UI 用户点得着 → 断言真驱动过控件` 与 `只是路由/纯函数层的断言` 是两件事，不要为了图省事把后者也标成 `（UI）`。
