...
1. 保存用户原始需求，不改写、不补充未经确认的产品定义——**哪怕当场就能改也要先落号**。
   2026-09-15 的教训：用户之前提过一次「巡检间隔改不了」，但那句话没有 INPUT 编号，
   收口时没人回头核对，缺陷留到下一轮用户自己再撞一次才被处理（`CR-20260915-
   process-hardening-flow`）。同一句话既是反馈也可能带着新要求时，先记原话再拆分
   （参照 `INPUT-2026-09-13-027` 的先例：用户一次答复里有三条不只是确认，各自带出
   了新要求，须另立 CR）。
...
4. P4 集成冒烟：必须用 Playwright 从真实浏览器入口验证主要操作流，并保存 real-entry 证据。
   **收口前列出这条清单本身**（路径 / 操作步骤 / 期望结果），写进 CR 的验收条件或证据
   文件——不是跑完了才记，是先写「打算验什么」再去验，验完逐条打勾。空泛的「真实入口：
   开场观感」这类描述事后没法回查到底验没验、验的是不是同一件事。
