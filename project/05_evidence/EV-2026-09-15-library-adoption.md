...
## 7. 真实入口（2026-09-15 当日补，用户自己那台）

服务：`npm run build:local` + `npm run serve:local`，端口 3000，真实可见 Chromium，真实 Provider。

**CP-3 审批面板**（`scripts/probe-library-panel.mjs`）：

```
改前：共 258 份，待采纳 258，已采纳 0，已拒绝 0
面板已在动态屏上出现
通过了：AIDC供电架构洞察报告.pdf
拒绝了：AI数据中心与电网洞察报告.pdf
改后：待采纳 254，已采纳 2，已拒绝 2
PASS 审批在真实入口上生效，且刷新后仍在
```

**CP-4 / CP-6 对话查阅**（`scripts/probe-library-chat.mjs`，同一个问题问两次）：

| | 模型的回答 |
|---|---|
| 采纳前 | 「读不到。原因：……目前处于待采纳区，按约定需审批通过后才可查阅……处理办法：在动态屏的『资料库』面板里把它采纳」 |
| 采纳后（`_辅助_bulletin.txt`） | 「RELIABILITY \| RESILIENCE \| SECURITY / POWERING TODAY. PROTECTING TOMORROW.」 |
| 采纳后（`aidc-grid.html`，165 KB 网页存档） | 「标题：AI 数据中心与电网；第一个小标题：七项核心判断」——CP-6 同此验毕 |

**验毕还原**：探针改过的 5 条全部设回待采纳（`258 / 0 / 0`），采纳时写下的索引卡随之删除——用户的审批队列与验证前一致。

## 8. 构建途中的两件事（记下来省得下次再查）

1. `npm run build:local` 连续两次崩在 webpack 的 `WasmHash._updateWithBuffer`（`Cannot read properties of undefined (reading 'length')`）。**与资料库无关**——把 `资料库/` 整个移出仓库再构建，同样崩。真因是第一次构建被中断后 `.next-prod` 留下了损坏的缓存：`rm -rf .next-prod` 后一次过。
2. 那次中断还把用户正在跑的 `serve:local` 一起弄停了（它服务的就是 `.next-prod`）。**改完代码要重建重启**这条纪律的另一面是：**重建会打断正在跑的那个服务**，所以重建后要把服务拉回来，并确认按需编译的路由真的 200。
