...
## 3. 真实入口（收口会话已执行，结果 PASS——但只覆盖 CP-1/CP-3）

- 来源: `scripts/probe-change-history-and-sources.mjs`
- 时间: 2026-09-18
- 采集者: 协调会话（claude-sonnet-5），针对用户本机 `npm run build:local && npm run serve:local`（端口 3000，全部 7 条 2026-09-18 批次 CR 均已合并）的真实生产构建服务
- 命令: `node scripts/probe-change-history-and-sources.mjs`
- 输出:
  ```
  已建立探针对象：探针测试友商-1789769579428
  API 能读回刚写入的历史=true
  消息清单渲染出可点击链接=true，href 正确=true
  弹窗打开前，内联输入框不存在=true
  点击「采集源设置」后弹窗打开=true
  「自动配置来源」按钮存在=true
  点击后对话框被预填正确文本，且未自动发送=true
  清理探针对象=true
  PASS 消息清单渲染正确、来源配置已收进弹窗、自动配置来源正确交给对话框
  ```
- 判定: **PASS**（CP-1、CP-3 全部核对项通过；一次性测试对象已用 `DELETE` 清理）

**这个探针没有覆盖 CP-2**：验的是"自动配置来源"按钮把预填文本交给对话框，没有让真实模型在对话里实际调用 `add_source` 工具本身。CP-2 自己的「变化点登记」发现方式一栏原本就只登记了机器（单测），未承诺真实入口——这不是本次收口漏做的事，只是如实标注哪一半真的被真实浏览器验证过。
