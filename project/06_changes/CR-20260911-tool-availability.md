...
- 状态: CLOSED（闭环完成：2 个任务 DONE；2026-09-14 按 DEC-270 ③ 收口。原记：APPROVED（快车道，DEC-021 ①：全部 CP 为双向门且每个 CP 的发现方式都是一条机器检查））
...
- R1 终裁: 已完成 | 用户
  - **R1 人工终裁**：分两部分，如实区分。
    - **CP-1/2/3 由用户直接提出并授权**，2026-09-11 原话：「另外模型要联网的时候，要让模型可以调用起来联网搜索。」——这既是缺陷报告也是修复授权，无需二次确认。
    - **CP-4/5（`.gitattributes` 与 BOM）未经用户单独拍板**。它们不是用户要的功能，而是交付 CP-1/2/3 时撞上的阻断性缺陷：BOM 使测试文件无法解析、CRLF 使 `tests/visual.test.ts` 整个套件无法加载——不修则本 CR 无法被验证。按「修复使交付可验证的阻断项」处理并**显式提请用户复核**：`.gitattributes` 会改变全仓库文件的行尾，属基础设施变更，用户若不认可可按「回滚方式」单独撤销该项而不影响 CP-1/2/3。
