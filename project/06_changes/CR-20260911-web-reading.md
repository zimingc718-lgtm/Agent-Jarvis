...
- 状态: CLOSED（闭环完成：1 个任务 DONE、3 条测试 PASS；2026-09-14 按 DEC-270 ③ 收口。原记：APPROVED（R1 人工终裁：用户在收到分析与两项待决后回复「都做吧」）→ P3/P4 完成）
...
在用户自己那台服务器上复验了本 CR 中**不花额度、不写数据**的那一段：`POST /api/settings/search/test` 返回「搜索服务连接正常。」——REQ-F-058 ④ 的「测试连接」在真实入口上成立。

**TEST-099 仍标 `isolated`**，没有改：它的主体（本机 SearXNG 出 10 条、两份 PDF 分别提取 33,675 与 230,992 字、opencompute.org 报人机校验未通过）需要一次真实的对话轮次才能在用户那侧重现，那要花他的 API 额度。**只验了一段就只记一段**，不把整条记录改成已在真实入口验过。
