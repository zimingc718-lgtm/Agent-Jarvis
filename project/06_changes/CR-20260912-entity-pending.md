...
- 影响测试: **小改** TEST-121（增 1 例）、TEST-088（增 ⑦，2026-09-13 补）
...
- 真实入口: **已执行**（2026-09-14，用户自己那台服务器 + 真实 Provider gpt-5）：模型自发调 `propose_entity_update`，来源不在已登记采集源内 → 入待采纳区 → 流上回 `entity_pending`，控制台出系统消息并刷新看板。证据：TEST-088；见 EV-2026-09-14-live-turns
- R1 终裁: 已完成 | 用户
...
| CP-2 | 产品 | **说一句 + 刷一次**：对话里出现一条系统消息，同时派发既有的 `KNOWLEDGE_CHANGED_EVENT`，看板据此刷新——不新增第二个事件名 | TASK-134 | 新增 | 双向 | 机器：TEST-088 ⑧（两种 `what` 说的是两句不同的话，且都派发刷新事件）；**真实入口**：在配好模型的真实对话里，模型自发提议时整条链路是否真的走通 |
...
- **2026-09-13 补**：客户端那条分支里能被机器守的一半已落成断言（TEST-088 ⑧）——当初记的阻塞理由是「`FloatingChat` 的测试文件正被并行会话改着」，那个理由早已消失，而这一半本来就不该靠人去看。
- **仍属人工发现**：在真实对话里模型自发提议时整条链路是否走通（CP-2 的后一半）。
