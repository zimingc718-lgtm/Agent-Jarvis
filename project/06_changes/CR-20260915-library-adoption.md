...
- 状态: CLOSED（闭环完成：TASK-390..420 全 DONE、TEST-390..420 全 PASS（含两条真实入口，用户自己那台）；2026-09-15 合入 main 收口。原记：APPROVED（R1 人工终裁：用户 2026-09-15 提出本需求并当日答复两项待裁定——入库范围「全部 253 MB 入库」、审批粒度「逐文件审批，界面在动态屏」））
...
- 真实入口: **已执行**（2026-09-15，用户自己那台服务器 `build:local` + `serve:local`，端口 3000，真实可见 Chromium + 真实 Provider）：①**CP-3** `scripts/probe-library-panel.mjs`——对话框入口唤起面板、逐条通过一条 / 拒绝一条、刷新后状态仍在（258 → 待采纳 254 / 已采纳 2 / 已拒绝 2）；②**CP-4** `scripts/probe-library-chat.mjs`——同一个问题，采纳前模型答「待采纳……在动态屏的『资料库』面板里把它采纳」，采纳后读出原文头两行；③**CP-6** 同一对照在 165 KB 的网页存档上再跑一遍，采纳后读出「AI 数据中心与电网 / 七项核心判断」（证据：TEST-410、TEST-420）。验毕**已把探针改过的 5 条全部还原为待采纳**（258 待采纳 / 0 / 0），索引卡随之删除。
