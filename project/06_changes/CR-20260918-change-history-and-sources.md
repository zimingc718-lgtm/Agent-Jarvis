...
- 状态: CLOSED（R1 已终裁，用户批量确认「Approve all three as-is」；已合并 main，`verify` PASS；真实入口已在生产构建上执行，PASS——覆盖 CP-1/CP-3；CP-2（`add_source` 工具本身）按其原始 CP 登记就只要求机器发现，未承诺真实入口，如实说明不是遗漏）
...
- 验收条件:
  ...
  - **真实入口**：已在用户本机生产构建上执行，PASS——①新建一次性对象、直接写一条历史 JSONL 后，卡片展开态渲染出这条消息，且是指向其 URL 的可点击链接；②展开态不再直接看到内联的"添加采集源"表单，改为一个"采集源设置"按钮；③点开按钮后是一个弹窗，弹窗内含既有的手动增删表单，以及一个"自动配置来源"按钮；④点击该按钮后，对话框输入框被正确预填指定文本、且没有被自动发送。全程用完清理（`DELETE /api/entities/<name>`），未污染真实看板。**这个探针没有覆盖的部分**：CP-2（`add_source` 工具本身）——验的是按钮把文本交给对话框，没有让真实模型实际调用这个工具；这一半的真实入口如实登记为尚未验证，不冒充已覆盖。
