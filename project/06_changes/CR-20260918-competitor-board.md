...
- 状态: CLOSED（R1 已终裁，用户批量确认「Approve as-is」；已合并 main，`verify` PASS；真实入口已在生产构建上执行确认，PASS）
...
- 验收条件:
  ...
  - **真实入口**：已在用户本机生产构建上执行，PASS——① 真实模型调用 `show_competitor_board` 后，展示屏切到友商看板，真实数据下 4 家已跟踪友商（台达-delta、施耐德电气、维谛技术、英维克）列头全部对得上；② 真实数据里没有两家友商共享同一参数名，共享维度核对如实跳过、未伪造场景；③ 没有登记参数的友商正确显示"未登记"，不报错、不留白。
