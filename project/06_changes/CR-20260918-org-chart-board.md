...
- 状态: CLOSED（R1 已终裁，用户批量确认「Approve all three as-is」；已合并 main，`verify` PASS；真实入口已在生产构建上执行确认，PASS，含协调会话独立核实清理成功）
...
- 验收条件:
  ...
  - **真实入口**：已在用户本机生产构建上执行，PASS——①对话把展示屏切到组织架构看板，真实响应；②对话用 `propose_person` 登记一位人员，来源域刻意用几乎不可能已注册的地址，模型正确回复提及待采纳/测试字样、未直接写入看板。探针自身通过对话请求模型忽略测试提议来清理，协调会话额外独立核实：直接查询 `/api/entities` 确认 `pending`/`proposals` 数组均为空——"维谛"对象没有留下任何编造的测试数据，清理是真的成功，不只是探针自称成功。
