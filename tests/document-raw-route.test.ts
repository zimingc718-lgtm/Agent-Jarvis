...
describe("/api/documents/raw", () => {
  ...
  it("⑤ 资料库里未采纳的文件 403，采纳后 200 且是 PDF 的 Content-Type", async () => {
    ...
    expect(shown.headers.get("content-type")).toContain("application/pdf");
    expect(await shown.text()).toBe("%PDF-1.4 fake");
  });
});
