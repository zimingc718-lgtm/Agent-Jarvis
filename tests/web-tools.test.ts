...
describe("web tools", () => {
  ...
  it("② 未配置地址时 web_search 不注册；总开关关闭时两个工具都不注册", () => {
    ...
    expect(webSearch.available({ ...context, searchConfigured: false, knowledgeCount: 0, contextWindow: 128_000 })).toBe(false);
    expect(readUrl.available({ ...context, searchConfigured: false, knowledgeCount: 0, contextWindow: 128_000 })).toBe(true);
